                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            <thead> <!--Judul Tabel-->
                                <tr>
                                    ...
                                    <th width="70%">
                                        Status Pernikahan
                                        <input list="op_nkh" class="form-control" type="search" id="co0" onkeyup="search(0)" placeholder="Cari Status Pernikahan...">
                                        <datalist id="op_nkh">
                                            <?php
                            ...
                            <tbody>
                                <?php
                                    $no = 0;
                                    $query = mysqli_query($koneksi, "SELECT * FROM `status_pernikahan` ORDER BY Status_Pernikahan ASC");
								...
								<?php echo "<tr id='change_form".$no."' style='display:none'>"; ?>
									<form method="post" enctype="multipart/form-data">
										...
										<td> <?php echo "<input type='text' class='form-control' placeholder='Status Pernikahan' name='na' value='".$nom['Status_Pernikahan']."'>"; ?></td>
										<td align=center>
											<?php echo "<button type='button' style='width:45%' class='btn btn-secondary' onclick='change(0, ".$no.")'>Batal</button>" ?>
											<?php echo "<button type='submit' style='width:45%' name='update".$nom['Id_Status_Pernikahan']."' class='btn btn-sa'>Simpan</button>"; ?>
										</td>
									</form>
									<?php
										$ch = "update".$nom['Id_Status_Pernikahan'];
										if (isset($_POST[$ch])){
											$sql=$koneksi->query("UPDATE `status_pernikahan`
												SET `Status_Pernikahan` = '$_POST[na]'
												WHERE `Id_Status_Pernikahan` = '$nom[Id_Status_Pernikahan]'
											");
											if($sql){
												echo "<script>window.location = 'index.php?page=status_pernikahan'</script>";
											}else{
												echo "<script>alert('Proses gagal'); window.location = 'index.php?page=status_pernikahan'</script>";
											}
										}
									?>
								</tr>
                                    ...
                                    ?>
                                        ...
                                        <?php echo "<tr id='delete".$no."' style='display: none;'>"; ?>
                                            <td colspan=3 align=center>
                                                ...
                                                <?php echo "<a class='btn btn-del' style='width:40%' href='hapus.php?id=".$nom['Id_Status_Pernikahan']."&type=status_pernikahan'>Ya</a>"; ?>
                                            </td>
                                        </tr>
                                    <?php }?>
								</tr>
                                ...
                                ?>    
                                        <?php
                                            $cmax = mysqli_query($koneksi, "SELECT max(Id_Status_Pernikahan) as bat FROM `status_pernikahan`");
                                            $nox = mysqli_fetch_array($cmax);
                                            $IDD = $nox['bat'];
                                            $ord = (int) substr($IDD, 2, 3);
                                            $ord++;
                                            $IDD = "SP".sprintf("%03s", $ord);
                                        ?>
                                        <tr id="add_button">
                                        ...
                                        <tr id="add_form" style="display: none">
                                            <form method="post" enctype="multipart/form-data">
                                                ...
                                                <td><input type="text" class="form-control" placeholder="Status Pernikahan" name="na"></td>
                                                <td align=center>
                                            ...
                                            <?php 
                                                if (isset($_POST['insert'])){
                                                    $sql=$koneksi->query("INSERT INTO `status_pernikahan`(Id_Status_Pernikahan, Status_Pernikahan) 
                                                        VALUES('$IDD', '$_POST[na]')
                                                    ");
                                                    if($sql){
                                                        echo "<script>window.location = 'index.php?page=status_pernikahan'</script>";
